...
try:
    df_contatos = pd.read_csv('df_contatos.csv')
except:
...
def menu():
    ...
    print("[6] SAIR")
    print('\nÉ RECOMENDADO LISTAR OS CONTATOS PRIMEIROS ANTES DE QUALQUER ALTERAÇÃO\n')
...
def add_contato(df_contatos):
    ...
    while res2.lower() == 'n':
        os.system('cls')
        head()
        num = input('DIGITE O NÚMERO DE TELEFONE COM DDD E SEM FORMATAÇÃO (EX: 8500000000): ')
        numero_formatado = f'({num[0]+num[1]}) 9{num[2]+num[3]+num[4]+num[5]}-{num[6]+num[7]+num[8]+num[9]}'
        print('CERTIFIQUE-SE DE QUE O TELEFONE POSSUI EXATOS 10 DÍGITOS')
    ...
    print(df_contatos)
    df_contatos.to_csv('df_contatos.csv', index=False)
...
def alterar_contato(df_contatos):
    ...
    if alt == 1:
        ...
        while res_telefone == 'n':
            ...
            head()
            print(df_contatos)
            novo_telefone = input('\nDIGITE O NÚMERO DE TELEFONE COM DDD E SEM FORMATAÇÃO (EX: 8500000000): ')
            numero_formatado = f'({novo_telefone[0]+novo_telefone[1]}) 9{novo_telefone[2]+novo_telefone[3]+novo_telefone[4]+novo_telefone[5]}-{novo_telefone[6]+novo_telefone[7]+novo_telefone[8]+novo_telefone[9]}'
